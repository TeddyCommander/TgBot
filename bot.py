...
def askAge(message):
    ...
    msg = bot.send_message(chat_id, 'Сколько статей парсить?', reply_markup=m.amount_markup)
...
def askAmount(message):
    ...
    text = message.text.lower()
    if not text.isdigit():
        msg = bot.send_message(chat_id, 'Количество статей должно быть числом. Введите корректно.')
        bot.register_next_step_handler(msg, askAmount)
        return
    if int(text) < 1 or int(text) > 10:
        msg = bot.send_message(chat_id, 'Количество статей должно быть > 0 и <= 10. Введите корректно.')
        bot.register_next_step_handler(msg, askAmount)
        return
    ...
    if task.mySource == 'top':
        output = parsernews.getTitlesFromTop(int(text), task.myFilter)
        
    else:
        output = parsernews.getTitlesFromAll(int(text), task.myFilter)
